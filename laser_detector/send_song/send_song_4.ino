...
BuzzerNote* binary_note_array[sizeof(melody)];
BuzzerDuration* binary_duration_array[sizeof(durations)];
...
void loop()
{
    ...
    {
        int note = static_cast<int>(melody[i]);
        int dur = static_cast<int>(durations[i]);

        binary_note_array[i] = intToBinary(note, dur);
    }
